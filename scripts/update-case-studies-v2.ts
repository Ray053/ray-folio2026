/**
 * Restructures both case studies into a design-portfolio narrative with
 * explicit User Research / flow-chart / design-system beats (not just prose),
 * using "[[diagram:key]]" markers that ProjectDetailPage swaps for the
 * matching component in app/(frontend)/[locale]/work/[slug]/page.tsx.
 * Run with:
 *   node --env-file=.env.local node_modules/tsx/dist/cli.mjs scripts/update-case-studies-v2.ts
 */
...
async function run() {
  ...
  if (groupBuy) {
    ...
    console.log('Updated group-buy-ops case study (v2)')
  } else {
  ...
  if (dance) {
    ...
    console.log('Updated streetdance-learning-platform case study (v2)')
  } else {
